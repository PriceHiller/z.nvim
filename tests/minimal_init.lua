...
---@class z.test.MinPlugin A plugin to download and register on the package path
---@alias z.test.PluginName string The plugin name, will be used as part of the git clone destination
---@alias z.test.PluginCloneInfo string | string[] The git url a plugin located at or a table of arguments to be passed to `git clone`
---@alias z.test.MinPlugins table<z.test.PluginName, z.test.PluginCloneInfo>

---Downloads a plugin from a given url and registers it on the 'runtimepath'
---@param plugin_name z.test.PluginName
---@param plugin_clone_args z.test.PluginCloneInfo
...
---@param plugins z.test.MinPlugins?
function M.setup(plugins)
    ...
    for plugin_name, plugin_clone_args in pairs(plugins or {}) do
        M.load_plugin(plugin_name, plugin_clone_args)
    end

    -- Ensure `z` is registed on the runtimepath, set it up, and initialize it
